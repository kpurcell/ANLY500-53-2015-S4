...
```
save.image(file="~/Desktop/workspace.Rdata")
save(x, file="~/Desktop/important.Rdata")  # For saving an object to an rdata file
...
## Assingment operator
- in `R` most actions are executed *expressions*
- often *expressions* assign a value to an *object*
- `R` uses the <-- symbol to assign values to *objects*
- `R` will accept the = sign for assignment as well
- Most style guides suggest using the <--
...
library(stargazer)
stargazer(ds, summary=FALSE,type="html")
```
